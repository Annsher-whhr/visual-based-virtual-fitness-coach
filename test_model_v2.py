...
if os.path.exists(scaler_path):
    ...
    print("[WARNING] 未找到标准化器，使用新的标准化器")
...
print(f"     正样本: {np.sum(y == 1)} ({np.sum(y == 1)/len(y)*100:.1f}%)")
print(f"     负样本: {np.sum(y == 0)} ({np.sum(y == 0)/len(y)*100:.1f}%)\n")
...
print(f"  F1分数 (F1-Score):  {train_f1:.4f} ({train_f1*100:.2f}%)")
print()
...
print(f"  F1分数 (F1-Score):  {test_f1:.4f} ({test_f1*100:.2f}%)")
print()
...
if overfit_gap > 0.1:
    print(f"  [!] 可能存在过拟合！训练集准确率比测试集高 {overfit_gap*100:.2f}%")
elif overfit_gap > 0.05:
    print(f"  [*] 轻微过拟合，训练集准确率比测试集高 {overfit_gap*100:.2f}%")
else:
    print(f"  [OK] 模型泛化良好，训练集和测试集准确率差异仅 {overfit_gap*100:.2f}%")
print()
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[0, 0],
            xticklabels=['错误动作', '正确动作'],
            yticklabels=['错误动作', '正确动作'])
...
bars1 = axes[1, 1].bar(x - width/2, train_metrics, width, label='训练集', color='skyblue')
bars2 = axes[1, 1].bar(x + width/2, test_metrics, width, label='测试集', color='orange')
...
plt.suptitle('太极拳起势动作评估模型 v2.0 - 评估报告 (20帧输入)', 
...
if test_accuracy >= 0.95:
    print("[OK] 模型表现优秀！可以放心使用。")
elif test_accuracy >= 0.85:
    print("[OK] 模型表现良好，但仍有优化空间。")
else:
    print("[!] 模型表现一般，建议考虑以下改进：")
